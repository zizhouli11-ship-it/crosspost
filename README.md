...
- 33 个单元测试全绿:数据模型、三个平台适配与校验、凭证存储、编排器隔离、三个发布器(API 调用以 mock 验证请求构造)。
- 服务端 HTTP 冒烟:`/api/platforms`、`/api/validate`(纯文字→ xhs:skip、x:ok、youtube:skip)、`/api/credentials/x`(保存后 x 由未连接→就绪)均符合预期。
...
2. X 配好密钥后发一条图文。
3. YouTube 授权后传一个视频。
4. 三平台合并发布:任一失败不影响其他,结果分别带链接/原因。
